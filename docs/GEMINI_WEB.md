...
`gemini-review` dùng Playwright và Chromium profile riêng (`~/.gemini/gemini-bridge/`) để gửi prompt đã sanitize lên `gemini.google.com/app` rồi lấy text response. Đây là **second-opinion reviewer** (không ghi approval state chính thức) — merge gate vẫn chỉ nhận approval từ ChatGPT (`templates/merge-approved-pr.sh`).
...
```bash
~/.gemini/gemini-bridge/bin/gemini-review login      # đăng nhập Google (1 lần, xử lý được 2FA/consent)
# ...hoặc tự động từ .env (không gõ tay) — xem docs/AUTO_LOGIN.md:
# fill ~/.gemini/gemini-bridge/.env (GEMINI_EMAIL/GEMINI_PASSWORD, chmod 600), rồi:
~/.gemini/gemini-bridge/bin/gemini-review login --auto
~/.gemini/gemini-bridge/bin/gemini-review status     # → {"profileExists":true,"loggedIn":true,"guestAvailable":false,"envConfigured":true}
printf '%s\n' 'cross-check kết quả task vừa review bằng ChatGPT: <summary>' | \
  ~/.gemini/gemini-bridge/bin/gemini-review ask
~/.gemini/gemini-bridge/bin/gemini-review reset      # xóa mapping repo+branch
~/.gemini/gemini-bridge/bin/gemini-review chats      # xem mapping
```
...
`ask` hỗ trợ `--new`, `--headless`, `--timeout=SECONDS`, `--file=PATH`. Mỗi repo+branch reuse một Gemini conversation (`identity:branch` qua `gh remote`, fallback `basename:branch`, legacy migration) cho đến khi vượt ngưỡng trong `~/.gemini/gemini-bridge/bridge-config.json` (`max_chars`/`max_turns`/`max_age_hours`). Lock `~/.gemini/gemini-bridge/.lock` với PID check + `Atomics.wait` bảo vệ profile.

Runtime state nằm dưới `~/.gemini/gemini-bridge/` (`profile/`, `chats.json` `0600`, `bridge-config.json`, `libs/`). Không copy hoặc commit browser profile. Không gửi credential, cookie, `.env`, key, raw diff, transcript.
...
~/.gemini/gemini-bridge/bin/gemini-review status   # → loggedIn:true
~/.gemini/gemini-bridge/bin/gemini-review chats
```

Rollback: thoát mọi `gemini-review`, `rm ~/.local/bin/gemini-review` (symlink), nếu muốn xóa login state backup rồi xóa riêng `~/.gemini/gemini-bridge/`. Không xóa toàn bộ `~/.config/opencode/`.
